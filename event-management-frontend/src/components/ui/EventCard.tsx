    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex flex-col space-y-2 mb-4">
          ...
          <div>
            <div className="flex items-center justify-between text-text-tertiary text-sm mb-1">
              ...
              <span className={`text-xs font-semibold ${event.isFull ? 'text-error' : 'text-green-400'}`}>
                {event.isFull ? 'Full' : 'Available'}
              </span>
            </div>
            <div className="w-full bg-surface/50 rounded-full h-1.5">
              <div 
                className={`h-1.5 rounded-full ${event.isFull ? 'bg-error' : 'bg-gradient-to-r from-primary to-secondary'}`}
        ...
        <Link href={`/events/${event.id}`} className="w-full">
          <motion.button 
            className="w-full bg-gradient-to-r from-primary to-secondary hover:from-primary-light hover:to-secondary-light text-white font-medium py-2 px-4 rounded-lg transition-all duration-300"
